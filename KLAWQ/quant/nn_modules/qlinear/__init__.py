...
class BaseQuantLinear(nn.Module):
    ...
    def __init__(self,
                 bits: int, group_size: int, desc_act: bool, sym: bool,
                 in_features: int, out_features: int, bias: bool,
                 pack_dtype: t.dtype, backend: BACKEND, name: str = None,
                 register_buffers: bool = False, register_buffers_in_features: int = None,
                 register_buffers_out_features: int = None, **kwargs):
        super().__init__()
        if name is None: name = f"{self.__class__.__module__}.{self.__class__.__qualname__}"
        self.name, self.in_features, self.out_features = name, in_features, out_features
        self.group_size, self.bits, self.desc_act = (group_size if group_size != -1 else in_features), bits, desc_act
        self.pack_dtype, self.backend, self.maxq = pack_dtype, backend, 2 ** self.bits - 1
        self.optimized = False
        if self.pack_dtype == t.int8: self.pack_dtype_bits, self.pack_np_dtype, self.pack_np_math_dtype = 8, np.int8, np.uint8
        elif self.pack_dtype == t.int16: self.pack_dtype_bits, self.pack_np_dtype, self.pack_np_math_dtype = 16, np.int16, np.uint16
        elif self.pack_dtype == t.int32: self.pack_dtype_bits, self.pack_np_dtype, self.pack_np_math_dtype = 32, np.int32, np.uint32
        elif self.pack_dtype == t.int64: self.pack_dtype_bits, self.pack_np_dtype, self.pack_np_math_dtype = 64, np.int64, np.uint64
        else: raise ValueError("Unsupported weight_dtype.")
        ...
        if register_buffers:
            in_f = self.in_features if not register_buffers_in_features else register_buffers_in_features
            out_f = self.out_features if not register_buffers_out_features else register_buffers_out_features
            padded_in_f = in_f
            if padded_in_f % self.pack_factor != 0:
                padded_in_f += self.pack_factor - (padded_in_f % self.pack_factor)
            self.register_buffer("qweight", t.zeros((padded_in_f // self.pack_factor, out_f), dtype=self.pack_dtype))
            self.register_buffer("qzeros", t.zeros((math.ceil(in_f / self.group_size), out_f // self.pack_factor), dtype=self.pack_dtype))
            self.register_buffer("scales", t.zeros((math.ceil(in_f / self.group_size), out_f), dtype=t.float16))
            self.register_buffer("g_idx", t.tensor([i // self.group_size for i in range(in_f)], dtype=t.int32))
            if bias: self.register_buffer("bias", t.zeros(out_f, dtype=t.float16))
            else: self.bias = None

    def list_buffers(self): return [b for name, b in self.named_buffers() if b is not None and t.is_tensor(b)]
